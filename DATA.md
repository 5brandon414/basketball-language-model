...
token's id is its index in the array. The shipped `data/vocab.json` is this
exact file — copy it to `<data-dir>/vocab.json` when building the corpus
(the loader reads it from `--data-dir`, not from the repo).
...
| sdiff | score diff (home-away) ENTERING the event — points through the previous row — / 20, clipped to [-2, 2] |
| actor | on-court slot of the acting player in the five in force before the event, ordered as in lineups.parquet: 0-4 home, 5-9 away; on S_SUB rows the outgoing player's slot in the pre-sub five; -100 if none |
...
Extra columns are ignored (the reference build carries a raw `dt` column
...
Multi-player substitutions share one timestamp and one stint change; every
event row at that timestamp maps to the new stint.
...
list. Every game in the splits must have a row here — the loader has no
fallback for a missing roster list.

One corpus game (0022501032) dresses 18 on one side; the truncation drops
its 18th sorted id. That game is labelled `exclude` in all three fold
splits, so no paper number touches it.
...
`--card` selects the card file, both for `train_bball_lm.py` and for the
trees baseline in `experiments/table2_margins.py`; the paper uses
`player_card_fold{k}.parquet` (49 dials) for both, matched to `--splits`.
The legacy `player_card.parquet` (15 dials, normalized on the default
split's window rather than per fold) reproduces neither table and runs a
different baseline.
...
License: MIT — the code and the shipped `data/` files (our own outputs)
alike; the corpus itself derives from publicly available NBA.com
endpoints and is not redistributed here.

## Sidecar tables (training inputs)

`prior_minutes.parquet` — one row per (player_id, game_id): `prior_min`,
the player's minutes averaged over their last 5 appearances strictly
before that game (shifted one game; first appearances carry 0). Feeds
`--prior` / `--budget-ch`.

`team_pace.parquet` — one row per game_id: `pace_z`, an as-of pace
scalar for the matchup, z-scored against a fixed pre-cutoff anchor.
Feeds `--pace-file` / `--pace-ch`. Both tables derive from the event
corpus alone and can be rebuilt from it; the shipped copies are the
ones the paper's checkpoints trained on.
