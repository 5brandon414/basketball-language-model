...
def load_corpus(data_dir, splits_file=None, fix_sub_boundary=False):
    ...
    for f in sorted(glob.glob(os.path.join(data_dir, "events_*.parquet"))):
        for gid, g in pd.read_parquet(f).groupby("game_id"):
            _n += 1
            if _n % 2000 == 0:
                print(f"  corpus load: {_n:,} games...", flush=True)
            ...
            if fix_sub_boundary:
                # Rows at a substitution boundary condition on the PRE-sub
                # five. A SUB token's clock equals the NEW stint's t_start,
                # so side="right" lands on the post-sub lineup; the same is
                # true for non-SUB events sharing that dead-ball timestamp
                # (made FT, timeout) that precede the swap in row order.
                # Rule: within each run of equal-clock rows, every row up to
                # and including the run's LAST SUB token gets the lineup an
                # epsilon before the boundary.
                is_sub = g.token.str.endswith("_SUB").to_numpy()
                idx_pre = np.clip(np.searchsorted(ts, el - 0.05, side="right") - 1,
                                  0, len(ts) - 1)
                run_id = np.concatenate([[0], np.cumsum(el[1:] != el[:-1])])
                pre = np.zeros(len(el), bool)
                pos = np.arange(len(el))
                for r in np.unique(run_id[is_sub]):
                    in_run = run_id == r
                    last_sub = pos[in_run & is_sub].max()
                    pre |= in_run & (pos <= last_sub)
                idx = np.where(pre, idx_pre, idx)
            R = 17
            hro, aro = avail[gid]
